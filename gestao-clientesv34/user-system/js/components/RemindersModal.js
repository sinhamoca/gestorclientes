...
function RemindersModal({ reminders, templates, onClose, onRefresh }) {
  ...
  const handleSendNow = async () => {
    if (selectedReminders.length === 0) {
      alert('Selecione pelo menos um lembrete para enviar.');
      return;
    }
    ...
    if (activeSelected.length === 0) {
      alert('Nenhum lembrete ativo selecionado.');
      return;
    }
    ...
    if (totalClientes === 0) {
      alert('Nenhum cliente será afetado pelos lembretes selecionados hoje.');
      return;
    }
    ...
    try {
      ...
      alert(`✅ Envio iniciado!\n\n${result.message}\n\nAcompanhe o progresso na página de Logs.`);
  ...
  const getDaysDescription = (days) => {
    ...
    return `${days} dia(s) depois (vencido)`;
  };
