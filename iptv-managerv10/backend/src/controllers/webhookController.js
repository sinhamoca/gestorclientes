...
async function handleUniplayRenewal(req, res, webhookData, plan) {
  const { 
    client_id,
    user_id, 
    client_name,          // ← Nome para buscar (CRÍTICO!)
  ...
  console.log(`   💳 Créditos: ${plan_duration_months} (1 mês = 1 crédito)`);
  console.log(`   🔍 Busca: Automática (P2P + IPTV)`);
  console.log(`   🔐 Proxy: Ativado (proxychains)`);

  // Criar serviço Uniplay COM PROXY (igual Sigma)
  ...
  try {
    ...
    console.log(`\n🔍 [WEBHOOK-UNIPLAY] Buscando cliente por nome: "${client_name}"`);
    
    const foundClient = await service.findClientByName(client_name);

    if (!foundClient) {
      console.error('\n❌ [WEBHOOK-UNIPLAY] CLIENTE NÃO ENCONTRADO!');
      console.log('='.repeat(60));
      ...
      return res.status(404).json({
        ...
        message: 'Cliente não foi encontrado no Uniplay (P2P ou IPTV). Verifique se o nome está EXATAMENTE igual ao cadastrado no sistema Uniplay.',
        suggestions: [
          'Verifique se o nome tem acentos corretos',
          'Confirme se não há espaços extras',
          'Certifique-se que o cliente existe no Uniplay'
        ]
      });
    }

    // ========== RENOVAR CLIENTE ==========
    console.log(`\n🔄 [WEBHOOK-UNIPLAY] Renovando ${plan_duration_months} crédito(s)...`);
    
    const resultado = await service.renewClient(
      foundClient.id,
      foundClient.serviceType,  // 'p2p' ou 'iptv'
      plan_duration_months
    );
    ...
    // ========== VERIFICAR RESULTADO ==========
    console.log(`\n${'='.repeat(60)}`);
    console.log('📊 [WEBHOOK-UNIPLAY] RESUMO DA RENOVAÇÃO');
    console.log('='.repeat(60));

    if (resultado.sucesso) {
      console.log('✅ [WEBHOOK-UNIPLAY] RENOVAÇÃO CONCLUÍDA COM SUCESSO!');
      console.log(`   Cliente: ${client_name}`);
      console.log(`   ID Uniplay: ${foundClient.id}`);
      console.log(`   Tipo: ${foundClient.serviceType.toUpperCase()}`);
      console.log(`   Créditos: ${plan_duration_months}`);
      console.log('='.repeat(60) + '\n');
      
      return res.json({
        success: true,
        provider: 'uniplay',
        message: 'Renovação automática Uniplay concluída com sucesso',
        data: {
          client_id: client_id,
          client_name: client_name,
          uniplay_internal_id: foundClient.id,
          service_type: foundClient.serviceType,
          credits_renewed: plan_duration_months,
          resultado: resultado
        }
      });
    } else {
      console.error('\n❌ [WEBHOOK-UNIPLAY] RENOVAÇÃO FALHOU!');
      console.log('Resultado:', resultado);
      console.log('='.repeat(60) + '\n');
      ...
      return res.status(500).json({
        success: false,
        provider: 'uniplay',
        message: 'Renovação automática Uniplay falhou',
        error: resultado.error,
        data: {
          client_id: client_id,
          client_name: client_name,
    ...
    return res.status(500).json({ 
      success: false,
      provider: 'uniplay',
      error: 'Erro ao processar renovação Uniplay',
      message: error.message 
    });
  }
}
