...
export async function renewClient(req, res) {
  ...
  const clientResult = await query(
    ...
    console.log(`   Is Uniplay: ${client.is_uniplay_plan || false}`);
    console.log(`   Is UniTV: ${client.is_unitv_plan || false}`);  // ← ADICIONAR
    ...
    if (renew_in_iptv) {
      ...
      if (!client.is_sigma_plan && 
      ...
      } else if (!client.username && !client.is_unitv_plan) {
        ...
        try {
          ...
          if (iptvRenewal.success) {
            ...
            // ========== 🆕 VINCULAR CÓDIGOS UNITV À TRANSAÇÃO ==========
            if (client.is_unitv_plan && 
                transaction && 
                iptvRenewal.unitv_code_ids && 
                iptvRenewal.unitv_code_ids.length > 0) {
              
              console.log(`🎫 Vinculando ${iptvRenewal.unitv_code_ids.length} código(s) UniTV à transação ${transaction.id}...`);
              
              for (const codeId of iptvRenewal.unitv_code_ids) {
                try {
                  await query(`
                    INSERT INTO transaction_unitv_codes (transaction_id, unitv_code_id)
                    VALUES ($1, $2)
                    ON CONFLICT DO NOTHING
                  `, [transaction.id, codeId]);
                } catch (linkError) {
                  console.error(`⚠️  Erro ao vincular código ${codeId}:`, linkError.message);
                }
              }
              
              console.log(`✅ Códigos vinculados à transação com sucesso!`);
            }
...
export async function getExpandedClientStats(req, res) {
  try {
    ...
    const expired30Result = await query(
      `SELECT COUNT(*) FROM clients 
       WHERE user_id = $1 
    ...
    const expired60Result = await query(
      `SELECT COUNT(*) FROM clients 
       WHERE user_id = $1 
    ...
    const expired90Result = await query(
      `SELECT COUNT(*) FROM clients 
       WHERE user_id = $1 
    ...
    // Total recebido no ano (de todas as transações pagas)
    const yearlyReceivedResult = await query(
    ...
    const monthlyReceivedResult = await query(
      `SELECT COALESCE(SUM(amount_received), 0) as total_received
       FROM financial_transactions
       WHERE user_id = $1
       AND status = 'paid'
    ...
    res.json({
      ...
      active: parseInt(activeResult.rows[0].count),
      expired: parseInt(expiredResult.rows[0].count),
      expired_30_days: parseInt(expired30Result.rows[0].count),
      expired_60_days: parseInt(expired60Result.rows[0].count),
      expired_90_days: parseInt(expired90Result.rows[0].count),
      total_received_year: parseFloat(yearlyReceivedResult.rows[0].total_received),
      total_received_month: parseFloat(monthlyReceivedResult.rows[0].total_received)
    });
  } catch (error) {
