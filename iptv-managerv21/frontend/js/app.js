...
function App() {
  ...
  //estados club
  const [hasClubCredentials, setHasClubCredentials] = useState(false);
  const [showClubCredentialsModal, setShowClubCredentialsModal] = useState(false);
  const [capturingClubClients, setCapturingClubClients] = useState(false);
  const [clubClients, setClubClients] = useState([]);
  const [clubClientStats, setClubClientStats] = useState(null);
  
  // Estados para sincronização Club
  const [clubSyncModalOpen, setClubSyncModalOpen] = useState(false);
  const [selectedClientForClubSync, setSelectedClientForClubSync] = useState(null);
  ...
  useEffect(() => {
    if (authenticated) {
      checkClubCredentials();
    }
  }, [authenticated]);

  // Listener para fechar modal Club
  useEffect(() => {
    const handleCloseClubModal = () => {
      setShowClubCredentialsModal(false);
    };
    
    window.addEventListener('closeClubModal', handleCloseClubModal);
    
    return () => {
      window.removeEventListener('closeClubModal', handleCloseClubModal);
    };
  }, []);

  async function checkClubCredentials() {
    try {
      const data = await club.getCredentials();
      setHasClubCredentials(data.hasCredentials || false);
      
      // Se tem credenciais, carregar clientes Club
      if (data.hasCredentials) {
        await loadClubClients();
      }
    } catch (error) {
      console.error('Erro ao verificar credenciais Club:', error);
    }
  }
  ...
  function handleOpenClubSyncModal(client) {
    setSelectedClientForClubSync(client);
    setClubSyncModalOpen(true);
  }

  async function handleSyncClubClient(clubClient) {
    if (!selectedClientForClubSync || !clubClient) return;

    if (!confirm(`Confirma sincronização?\n\nCliente: ${selectedClientForClubSync.name}\nClub ID: ${clubClient.client_id}\nClub Nome: ${clubClient.reseller_notes || clubClient.username}`)) {
      return;
    }
    ...
    try {
      console.log('🔄 [CLUB SYNC] Sincronizando:', {
        clientId: selectedClientForClubSync.id,
        clubClientId: clubClient.client_id
      });

      await api.syncClientWithClub(selectedClientForClubSync.id, clubClient.client_id);

      setSuccess(`✅ Cliente ${selectedClientForClubSync.name} sincronizado com Club!\n\nID Club: ${clubClient.client_id}\nNome: ${clubClient.reseller_notes || clubClient.username}`);

      setClubSyncModalOpen(false);
      await loadClients();

    } catch (error) {
      console.error('❌ [CLUB SYNC] Erro:', error);
  ...
  async function handleCaptureClubClients() {
    if (!hasClubCredentials) {
      alert('⚠️ Configure suas credenciais Club primeiro!');
      return;
    }

    if (!confirm('🚀 Iniciar captura de clientes do painel Club?\n\n⚠️ Isso pode levar alguns minutos devido ao captcha.')) {
      return;
    }

    setCapturingClubClients(true);
    setError(null);
    setSuccess(null);
    ...
    try {
      ...
      const result = await club.captureClients();
      
      console.log('✅ [CLUB] Captura concluída:', result);
      
      // Mostrar mensagem de sucesso
      setSuccess(`✅ ${result.captured} clientes Club capturados e salvos com sucesso!\n\n📊 Total no painel: ${result.total}\n💾 Salvos localmente: ${result.captured}`);
      
      // Recarregar lista de clientes Club
      await loadClubClients();
      
      // Mudar para tab de clientes Club
      setActiveTab('club-clients');
      
    } catch (error) {
      console.error('❌ [CLUB] Erro ao capturar clientes:', error);
      setError(`Erro ao capturar clientes Club: ${error.message}`);
    } finally {
      setCapturingClubClients(false);
    }
  }
  ...
  return h('div', { className: 'min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100' },
      ...
      h('div', { className: 'max-w-7xl mx-auto px-4 py-4' },
        h('div', { className: 'flex items-center justify-between flex-wrap gap-4' },
          ...
          h('div', { className: 'flex items-center gap-2 flex-wrap' },
            ...
            //botao credenciais club
            h('button', {
              onClick: () => setShowClubCredentialsModal(true),
              className: `px-4 py-2 rounded-lg flex items-center gap-2 transition ${
                hasClubCredentials 
                  ? 'bg-purple-600 hover:bg-purple-700 text-white' 
                  : 'bg-yellow-500 hover:bg-yellow-600 text-white'
              }`
            }, hasClubCredentials ? '✓ Credenciais Club' : '🔑 Credenciais Club'),

            // Botão Capturar Clientes Club
            h('button', {
              onClick: handleCaptureClubClients,
              disabled: capturingClubClients || !hasClubCredentials,
            ...
            // Botão credenciais rush
            h(RushCredentialsModal),
    ...
    h('nav', { className: 'bg-white border-b' },
      h('div', { className: 'max-w-7xl mx-auto px-4' },
        h('div', { className: 'flex gap-2' },       
          ...
          h('button', {
            onClick: () => setActiveTab('club-clients'),
            className: `px-4 py-3 font-medium transition ${activeTab === 'club-clients' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-600 hover:text-gray-800'}`
          }, '🎯 Clientes Club')
        )
      )
    ),
    ...
    h('main', { className: 'max-w-7xl mx-auto px-4 py-8' },
      ...
      activeTab === 'club-clients' && h('div', null,
        h('div', { className: 'mb-6' },
          h('h2', { className: 'text-2xl font-bold text-gray-800 mb-2 flex items-center gap-2' }, 
            '🎯 Clientes Club',
            h('span', { className: 'text-base font-normal text-gray-600' }, '(dashboard.bz)')
          ),
          clubClientStats && h('div', { className: 'grid grid-cols-2 md:grid-cols-3 gap-4' },
            h('div', { className: 'bg-purple-50 p-4 rounded-lg shadow' },
              h('p', { className: 'text-gray-600 text-sm' }, 'Total de Clientes'),
              h('p', { className: 'text-2xl font-bold text-purple-600' }, clubClientStats.total)
            ),
            h('div', { className: 'bg-blue-50 p-4 rounded-lg shadow' },
              h('p', { className: 'text-gray-600 text-sm' }, 'Última Captura'),
              h('p', { className: 'text-sm font-medium text-blue-600' }, 
                clubClients.length > 0 
                  ? new Date(clubClients[0].captured_at).toLocaleString('pt-BR')
                  : 'Nunca'
              )
            ),
            h('div', { className: 'bg-gray-50 p-4 rounded-lg shadow' },
              ...
              }, capturingClubClients ? '⏳ Capturando...' : '🔄 Recapturar')
            )
          )
        ),

        // Lista de clientes Club
        h('div', { className: 'bg-white rounded-lg shadow overflow-hidden' },
          clubClients.length === 0 ? (
            h('div', { className: 'text-center py-12' },
              h('div', { className: 'text-6xl mb-4' }, '📭'),
              h('p', { className: 'text-gray-500 text-lg mb-2' }, 'Nenhum cliente Club capturado'),
              h('p', { className: 'text-gray-400 text-sm mb-4' }, 'Clique em "Capturar Clientes Club" no topo da página'),
              h('button', {
                onClick: handleCaptureClubClients,
                disabled: !hasClubCredentials || capturingClubClients,
                className: 'px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white rounded-lg font-medium transition'
              }, hasClubCredentials ? '📥 Capturar Agora' : '🔑 Configure Credenciais Primeiro')
            )
          ) : (
              ...
              h('table', { className: 'w-full' },
                h('thead', { className: 'bg-gray-50 border-b' },
                  h('tr', null,
                    ...
                    h('th', { className: 'px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase' }, 'Username'),
                    h('th', { className: 'px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase' }, 'Nome'),
                    h('th', { className: 'px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase' }, 'Status'),
                    h('th', { className: 'px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase' }, 'Vencimento'),
                ...
                h('tbody', { className: 'divide-y divide-gray-200' },
                  clubClients.map(client =>
                    h('tr', { key: client.id, className: 'hover:bg-gray-50 transition' },
                      h('td', { className: 'px-4 py-3 text-sm text-gray-900' }, client.client_id),
                      h('td', { className: 'px-4 py-3 text-sm font-medium text-gray-900' }, client.username),
                      h('td', { className: 'px-4 py-3 text-sm text-gray-600' }, client.reseller_notes || '-'),
                      h('td', { className: 'px-4 py-3 text-sm' },
                        h('span', {
                          className: `px-2 py-1 rounded text-xs font-medium ${
                            client.status === 'active' || client.status === 'Active'
                              ? 'bg-green-100 text-green-800'
                              : client.status === 'expired' || client.status === 'Expired'
                              ? 'bg-red-100 text-red-800'
                      ...
                      h('td', { className: 'px-4 py-3 text-sm text-gray-600' }, 
                        client.exp_date 
                          ? new Date(client.exp_date).toLocaleDateString('pt-BR')
                          : '-'
                      ),
                      h('td', { className: 'px-4 py-3 text-sm text-gray-600' }, client.conexoes || '-')
                    )
                  )
                )
              )
            ...
            clients.map(client =>
              h('div', { key: client.id, className: 'bg-white p-6 rounded-lg shadow hover:shadow-md transition' },
                ...
                h('div', { className: 'flex gap-2 flex-wrap' },
                  ...
                  // BOTÃO SINCRONIZAR CLUB (NOVO!)
                  h('button', {
                    onClick: () => handleOpenClubSyncModal(client),
      ...
      // Modal Sincronização Club (NOVO!)
      clubSyncModalOpen && h(ClubSyncModal, {
        isOpen: clubSyncModalOpen,
        onClose: () => !syncingClub && setClubSyncModalOpen(false),
        client: selectedClientForClubSync,
        onSync: handleSyncClubClient,
        syncing: syncingClub
      }),

      // Modal Credenciais Club
      showClubCredentialsModal && h(ClubCredentialsModal)
    )
  );
}

// Componente Modal Credenciais Club
function ClubCredentialsModal() {
  ...
  async function loadClubCredentials() {
    try {
      const data = await club.getCredentials();
      if (data.hasCredentials) {
        setHasCredentials(true);
  ...
  async function handleSaveCredentials() {
    ...
    try {
      await club.saveCredentials(username, password);
  ...
  return h('div', {
    ...
    onClick: (e) => {
      if (e.target === e.currentTarget) {
        // Fechar modal
        const event = new CustomEvent('closeClubModal');
        window.dispatchEvent(event);
      }
    }
  },
    h('div', { className: 'bg-white rounded-lg shadow-xl max-w-md w-full p-6' },
      // Header
      h('div', { className: 'flex items-center justify-between mb-4' },
        h('h3', { className: 'text-xl font-bold text-gray-800 flex items-center gap-2' },
          '🔑 Credenciais Club (dashboard.bz)'
        ),
        h('button', {
          onClick: () => {
            const event = new CustomEvent('closeClubModal');
      ...
      // Descrição
      h('div', { className: 'mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg' },
        h('p', { className: 'text-sm text-blue-800' },
          '🌐 Credenciais do painel dashboard.bz'
        ),
        h('p', { className: 'text-sm text-blue-700 mt-1' },
          '⚠️ Login usa hCaptcha - pode levar ~30s'
        )
      ),
      ...
      h('div', { className: 'space-y-4' },
        ...
        h('div', { className: 'flex gap-3 pt-4' },
          ...
          h('button', {
            ...
            disabled: saving,
            className: 'flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white rounded-lg transition'
