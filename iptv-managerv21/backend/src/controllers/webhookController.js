...
async function handlePainelFodaRenewal(req, res, webhookData, plan) {
  const { 
    ...
    user_id, 
    client_name,
    username,  // "tela 1, tela 2" ou vazio
    plan_duration_months  // ⚠️ ESTE CAMPO SERÁ IGNORADO!
  } = webhookData;
  ...
  if (!username || username.trim() === '') {
    // Cenário A: SEM username - renovar apenas o nome do cliente
    console.log('\n📋 [WEBHOOK-PAINELFODA] Cenário: Cliente único');
    console.log(`   → Buscar: "${client_name}"`);
    console.log(`   → Conexões: ${plan.num_screens}`);
    telasParaRenovar = [{ nome: client_name, connections: plan.num_screens }];
  } else {
    // Cenário B: COM username - loop de telas
    const telas = username
      .split(',')
      .map(t => t.trim())
      .filter(t => t.length > 0);
    
    console.log('\n📋 [WEBHOOK-PAINELFODA] Cenário: Múltiplas telas');
    ...
    telasParaRenovar = telas.map((tela, index) => {
      ...
      return { nome: nomeCompleto, connections: plan.num_screens };
    });
  }
  ...
  try {
    ...
    for (let i = 0; i < telasParaRenovar.length; i++) {
      const { nome, connections } = telasParaRenovar[i];
      
      console.log('─'.repeat(60));
      console.log(`🔄 [WEBHOOK-PAINELFODA] Renovando tela ${i + 1}/${telasParaRenovar.length}`);
      console.log(`   Cliente: "${nome}"`);
      console.log('─'.repeat(60));

      // ========== BUSCAR CLIENTE ==========
      const cliente = service.findClientByName(nome);
      
      if (!cliente) {
        console.error(`\n❌ [WEBHOOK-PAINELFODA] Cliente "${nome}" NÃO encontrado!`);
        console.log('─'.repeat(60) + '\n');
        
        // PARA TUDO no primeiro erro
        return res.status(404).json({
          success: false,
          provider: 'painelfoda',
      ...
      console.log(`   Expira: ${cliente.exp_date}\n`);
...
async function handleRushRenewal(req, res, webhookData, plan) {
  const { 
    ...
    cloudnation_id,        // Sufixos (tela 1, tela 2...) ou vazio
    plan_duration_months   // Quantidade de meses
  } = webhookData;
  ...
  try {
    ...
    // Verificar se tem sufixos (múltiplos clientes)
    const temSufixos = cloudnation_id && cloudnation_id.trim().length > 0;

    if (temSufixos) {
      // ===== RENOVAÇÃO MÚLTIPLA =====
      console.log('\n🔄 [WEBHOOK-RUSH] Modo: RENOVAÇÃO MÚLTIPLA');
      
      resultado = await service.renovarMultiplosClientes(
        client_name,                    // Nome base
        cloudnation_id,                 // Sufixos
        plan_duration_months,           // Meses
        rushType,                       // IPTV ou P2P
        plan.num_screens || 1           // Telas (só IPTV)
      );
