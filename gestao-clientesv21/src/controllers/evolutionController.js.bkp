...
export async function deleteInstance(req, res) {
  try {
    ...
    console.log(`🗑️  [User ${userId}] Excluindo instância: ${instance.instance_name}`);
    ...
    try {
      ...
      console.error('   ⚠️  Erro ao deletar da Evolution API:', error.message);
    }
    ...
    console.log(`   ✅ Instância removida do banco de dados`);
...
export async function sendTextMessage(instanceName, phoneNumber, message) {
  try {
    ...
    const response = await fetch(`${EVOLUTION_API_URL}/message/sendText/${instanceName}`, {
      ...
      headers: getHeaders(),
      body: JSON.stringify({
        number: formattedNumber,  // ← Envia apenas o número puro, sem @s.whatsapp.net
        text: message,
        delay: 1200
      })
    });
