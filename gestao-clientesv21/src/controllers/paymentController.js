...
// ========== PROCESSAR PAGAMENTO APROVADO (VERSÃO ATUALIZADA) ==========
async function processApprovedPayment(payment, session) {
  try {
    ...
    // ========== BUSCAR INFORMAÇÕES COMPLETAS (COM is_live21_plan) ==========
    ...
    console.log(`   Is Live21: ${clientData.is_live21_plan || false}`);  // ← NOVO LOG!
    console.log(`   Is Koffice: ${clientData.is_koffice_plan || false}`);  // ← NOVO
    ...
    // ========== 4. WEBHOOK PARA RENOVAÇÃO AUTOMÁTICA (COM is_live21_plan) ==========
    try {
