...
const api = {
  ...
  sendReminderToClient: (clientId) => api.request(`/reminders/send-to-client/${clientId}`, {
    method: 'POST'
  }),

  // 🆕 Enviar fatura via WhatsApp
  sendInvoice: (clientId) => api.request(`/clients/${clientId}/send-invoice`, {
    method: 'POST'
  }),
  ...
  getNotificationSettings: () => api.request('/whatsapp/notification-settings'),
  saveNotificationSettings: (data) => api.request('/whatsapp/notification-settings', {
    method: 'POST',
    body: JSON.stringify(data)
  }),

  // 🆕 ROTAS DE FILA DE RETRY
  getRetryQueue: () => api.request('/whatsapp/retry-queue'),
  manualRetry: (id) => api.request(`/whatsapp/retry-queue/${id}/retry`, {
    method: 'POST'
  }),
  removeFromRetryQueue: (id) => api.request(`/whatsapp/retry-queue/${id}`, {
    method: 'DELETE'
  }),

  // 🆕 ROTAS DE CONFIGURAÇÕES DE CHAMADAS E PRESENCE
  getCallSettings: () => api.request('/whatsapp/call-settings'),
  saveCallSettings: (data) => api.request('/whatsapp/call-settings', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
  ...
  // 🆕 SALVAR CHAVE PIX MANUAL
  savePixKey: (data) => api.request('/payment-settings/pix', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
