...
function PlansModal({ plans, onClose, onRefresh }) {
  ...
  const handleCapturePackages = async () => {
    ...
    if (!formData.painelfoda_domain || !formData.painelfoda_username || !formData.painelfoda_password) {
      toast.warning('Preencha domínio, usuário e senha antes de capturar os packages');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(`${data.packages.length} packages capturados!`);
      } else {
      ...
      toast.error(`Erro ao capturar packages: ${error.message}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onRefresh();
      toast.success(wasEditing ? 'Plano atualizado!' : 'Plano criado!');
    } catch (error) {
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-100px)]">
          <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="mb-4 p-3 bg-purple-50 border border-purple-200 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 cursor-pointer" onClick={() => {
                  const isChecked = !formData.is_sigma_plan;
                  setFormData({
                    ...formData,
                    is_sigma_plan: isChecked,
                    is_live21_plan: isChecked ? false : formData.is_live21_plan,
            ...
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-3 cursor-pointer" onClick={(e) => e.target.tagName !== "INPUT" && e.currentTarget.querySelector("input")?.click()}>
                <CustomCheckbox
            ...
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-start gap-3 cursor-pointer" onClick={(e) => {
                if (e.target.tagName !== 'INPUT' && !e.target.closest('.jelly-checkbox')) {
                  const isChecked = !formData.is_painelfoda_plan;
                  setFormData({
                    ...formData,
