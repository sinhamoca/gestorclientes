...
function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    loadData();
  }, [debouncedSearch, statusFilter, currentPage, startDate, endDate, serverFilter, planFilter, paymentTypeFilter]);
  ...
  const handleCopyInvoice = async (client) => {
    ...
    try {
      ...
      toast.success(`Link copiado: ${client.name}`);
    } catch (error) {
  ...
  const loadData = async () => {
    try {
      const [clientsData, statsData, plansData, serversData, templatesData, remindersData] = await Promise.all([
        api.getClients({ search: debouncedSearch, status: statusFilter, startDate, endDate, serverId: serverFilter, planId: planFilter, payment_type: paymentTypeFilter, page: currentPage, limit: 20 }),
  ...
  const handleSendReminder = async (client) => {
    if (!confirm(`📨 Enviar lembrete para ${client.name}?\n\nO sistema vai verificar se este cliente se enquadra em algum lembrete configurado para hoje.`)) {
      return;
    }

    try {
      const result = await api.sendReminderToClient(client.id);
      
      if (result.success) {
        toast.success(result.message, { 
          title: `Lembrete enviado: ${result.client_name}`,
          duration: 5000 
        });
      } else {
        toast.warning(result.message, { title: result.client_name });
      }
    } catch (error) {
      toast.error(error.message);
    }
  ...
  const handleSendInvoice = async (client) => {
    if (!client.payment_token) {
      toast.warning('Este cliente não possui link de pagamento gerado.');
      return;
    }

    if (!confirm(`Enviar fatura para ${client.name} via WhatsApp?`)) {
      return;
    }

    try {
      const result = await api.sendInvoice(client.id);
      toast.success(result.message);
    } catch (error) {
      toast.error(error.message);
    }
  ...
  const getStatusBadge = (client) => {
    ...
    if (client.expires_today) return React.createElement('span', {className: 'badge-neon bg-orange-500/20 text-orange-400 neon-border border-orange-500'}, '⚠️ Vence Hoje');
        ...
        <div className="max-w-7xl mx-auto px-3 md:px-6 py-3 md:py-5">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2 md:gap-3">
              {/* 1. Financeiro */}
              <button 
                onClick={() => window.location.href = '/financial.html'} 
                  ...
                  <>
                    ...
                    <div className="absolute left-0 md:right-0 md:left-auto top-full mt-2 w-56 bg-slate-800 rounded-lg shadow-lg border border-slate-700 z-20">
                      ...
                      <button
                        ...
                        className="w-full px-4 py-3 text-left hover:bg-slate-700 flex items-center gap-3 text-sm text-slate-300 transition"
                      >
                        <i data-lucide="file-text" className="w-4 h-4 text-indigo-400"></i>
                      ...
                      {/* Códigos UniTV */}
                      <button
                        onClick={() => { setShowModal('unitv'); setShowMobileMenu(false); }}
                        className="w-full px-4 py-3 text-left hover:bg-slate-700 flex items-center gap-3 text-sm text-slate-300 transition"
                      >
                        <i data-lucide="ticket" className="w-4 h-4 text-purple-400"></i>
                        <span>Códigos UniTV</span>
                      </button>
                      ...
                      <button
                        onClick={() => {
                          const token = localStorage.getItem('user_token');
                          window.open(`https://playlists.comprarecarga.shop?token=${token}`, '_blank');
                          setShowMobileMenu(false);
                        }}
                        className="w-full px-4 py-3 text-left hover:bg-slate-700 flex items-center gap-3 text-sm text-slate-300 transition"
                      >
                        <i data-lucide="list" className="w-4 h-4 text-cyan-400"></i>
                        <span>Gerenciar Playlists IPTV</span>
                      </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4 md:gap-6 mb-6 md:mb-10">
            ...
            <div className="gradient-card-2 stat-card-hover">
              <div className="relative z-10">
                ...
                <div className="text-xs md:text-sm font-bold text-white/90 uppercase tracking-wider">Lucro Ano</div>
              </div>
            </div>
            ...
            <div className="gradient-card-1 stat-card-hover">
              <div className="relative z-10">
                ...
                <div className="text-xs md:text-sm font-bold text-white/90 uppercase tracking-wider">Lucro Mês</div>
              </div>
            </div>
                  ...
                  clients.map(client => (
                    <tr key={client.id}>
                      <td className="px-4 md:px-6 py-4 md:py-5">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-white text-sm md:text-base">{client.name}</span>
                            {client.payment_type === 'pix' ? (
                              <span 
                                className="px-2 py-0.5 bg-orange-500/20 text-orange-400 rounded text-xs font-medium"
                                title="PIX Manual - Renovação manual"
                              >
                                🔑
                              </span>
                            ) : (
                              <span 
                                ...
                                title="Link Mercado Pago - Renovação automática"
                              >
                                🔗
                              </span>
                            )}
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (phone) {
                                ...
                                toast.warning('Cliente não possui WhatsApp cadastrado');
                              }
                            }}
                          ...
                          <div className="relative group">
                            ...
                            <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 rounded-lg shadow-lg border border-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                              ...
                              >
                                <i data-lucide="bell" className="w-4 h-4 text-yellow-400"></i>
                              ...
                              <button
                                ...
                                className="w-full px-4 py-2.5 text-left hover:bg-slate-700 flex items-center gap-3 text-sm text-slate-300 transition"
                              >
                                <i data-lucide="send" className="w-4 h-4 text-green-400"></i>
