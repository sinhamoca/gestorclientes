...
function ClientModal({ client, plans, servers, onClose, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    payment_type: client?.payment_type || 'link',  // ← ADICIONAR (padrão: link)
    skip_number_validation: client?.skip_number_validation || false  // 🌍 Número sem tratamento
  });
  ...
  const handleWhatsAppChange = (e) => {
    ...
    // 🌍 Se skip_number_validation estiver marcado, não valida
    if (formData.skip_number_validation) {
      setWhatsappError('');
      setWhatsappFormatted(value ? `📱 ${value.replace(/\D/g, '')}` : '');
      return;
    }
    
    // Validação em tempo real (apenas números BR)
    if (value.length > 0) {
  ...
  const handleWhatsAppBlur = () => {
    // 🌍 Se skip_number_validation, apenas remove caracteres não numéricos
    if (formData.skip_number_validation) {
      if (formData.whatsapp_number) {
        const cleaned = formData.whatsapp_number.replace(/\D/g, '');
        setFormData({...formData, whatsapp_number: cleaned});
        setWhatsappFormatted(cleaned ? `📱 ${cleaned}` : '');
      }
      return;
    }
    
    // Ao sair do campo, normaliza o número (apenas números BR)
    if (formData.whatsapp_number) {
  ...
  // Handler para toggle do checkbox skip_number_validation
  const handleSkipValidationChange = (e) => {
    const checked = e.target.checked;
    setFormData({...formData, skip_number_validation: checked});
    
    // Limpa erros quando ativa o modo sem tratamento
    if (checked) {
      setWhatsappError('');
      const cleaned = formData.whatsapp_number?.replace(/\D/g, '') || '';
      setWhatsappFormatted(cleaned ? `📱 ${cleaned}` : '');
    } else {
      // Revalida quando desativa
      if (formData.whatsapp_number) {
        const validation = validateWhatsAppNumber(formData.whatsapp_number);
        if (validation.valid) {
          setWhatsappError('');
  ...
  const handleSubmit = async (e) => {
    ...
    let whatsappToSend = formData.whatsapp_number;
    
    // 🌍 Se skip_number_validation, apenas limpa o número
    if (formData.skip_number_validation) {
      whatsappToSend = formData.whatsapp_number?.replace(/\D/g, '') || '';
      ...
      // Validação final do WhatsApp antes de enviar (apenas números BR)
      const whatsappValidation = validateWhatsAppNumber(formData.whatsapp_number);
      if (!whatsappValidation.valid) {
        setWhatsappError(whatsappValidation.error);
        toast.error(`WhatsApp inválido: ${whatsappValidation.error}`);
        return;
      }
      whatsappToSend = whatsappValidation.number;
    }
    ...
    try {
      ...
      const dataToSend = {
        ...formData,
        whatsapp_number: whatsappToSend
      };
  ...
  useEffect(() => {
    if (client?.whatsapp_number) {
      // 🌍 Se skip_number_validation, não valida
      if (client?.skip_number_validation) {
        setWhatsappFormatted(`📱 ${client.whatsapp_number}`);
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] flex flex-col">
        ...
        {/* Conteúdo com scroll */}
        <form id="clientForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Dados Básicos */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {/* Dica de formato */}
              {!formData.skip_number_validation && (
                <p className="text-gray-400 text-xs mt-1">
                  Formato: 55 + DDD + Número (12 ou 13 dígitos)
                </p>
              )}
              
              {/* 🌍 Checkbox: Número sem tratamento */}
              <div className="jelly-checkbox-wrapper mt-2">
                <CustomCheckbox
                  checked={formData.skip_number_validation}
                  onChange={handleSkipValidationChange}
                  color="orange"
                  label="🌍 Número sem tratamento (internacional ou formato específico)"
                />
              </div>
              
              {/* Aviso quando checkbox marcado */}
              {formData.skip_number_validation && (
                <div className="mt-2 p-2 bg-orange-50 border border-orange-200 rounded-lg">
                  <p className="text-xs text-orange-700">
                    <strong>⚠️ Atenção:</strong> O número será salvo exatamente como digitado, sem validação ou correção automática do 9º dígito. 
                    A responsabilidade de digitar corretamente é sua.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <input
                  ...
                  name="payment_type"
                  value="link"
                  checked={formData.payment_type === 'link'}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Mercado Pago - Renovação automática após pagamento
                  </p>
                </div>
              </label>
              ...
              >
                <input
                  ...
                  name="payment_type"
                  value="pix"
                  checked={formData.payment_type === 'pix'}
                  onChange={(e) => setFormData({...formData, payment_type: e.target.value})}
                  className="mt-1"
                />
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Chave PIX simples - Renovação manual
                  </p>
                </div>
              </label>
            </div>
            ...
            {formData.payment_type === 'pix' && (
              <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                <p className="text-sm text-orange-700">
                  <strong>⚠️ Atenção:</strong> Clientes com PIX Manual não recebem link de pagamento.
                  Configure sua chave PIX em <em>Configurações → Mercado Pago</em> para usar a variável <code className="bg-white px-1 rounded">{'{pix}'}</code> nos templates.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex justify-end gap-3 p-6 border-t flex-shrink-0 bg-gray-50">
          ...
          <button
            type="submit"
            form="clientForm"
            disabled={loading || (!!whatsappError && !formData.skip_number_validation)}
            className={`px-6 py-2 text-white rounded-lg transition disabled:opacity-50 ${
              (whatsappError && !formData.skip_number_validation)
