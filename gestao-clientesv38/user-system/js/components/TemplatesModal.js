/* ========================================
   TEMPLATES MODAL COMPONENT - CORRIGIDO
   Usa chaves simples: {nome}, {dias}, {fatura}
   ======================================== */
...
function TemplatesModal({ templates, onClose, onRefresh }) {
  ...
  // Mensagem padrão com chaves simples
  const defaultMessage = `Olá {nome}! 👋

Passando para lembrar que sua assinatura vence em *{dias} dia(s)*, no dia *{vencimento}*.

📦 Plano: {plano}
💰 Valor: {valor}

Para renovar, acesse o link abaixo:
{fatura}
...
Qualquer dúvida, estamos à disposição! 😊`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', type: 'vencimento_dia', message: defaultMessage });
      onRefresh();
      toast.success(wasEditing ? 'Template atualizado!' : 'Template criado!');
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                <button type="button" onClick={() => insertVariable('{dias}')} className="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded font-semibold">{'{dias}'} 📅</button>
                <button type="button" onClick={() => insertVariable('{fatura}')} className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded font-semibold">{'{fatura}'} 💳</button>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                💡 Atalhos disponíveis: <strong>{'{nome}'}</strong>, <strong>{'{vencimento}'}</strong>, <strong>{'{valor}'}</strong>, <strong>{'{plano}'}</strong>, <strong>{'{servidor}'}</strong>, <strong>{'{dias}'}</strong> e <strong>{'{fatura}'}</strong> (link de pagamento)
              </p>
            </div>
