...
function RenewalModal({ client, onClose, onSuccess }) {
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      let mainMessage = `Nova data: ${new Date(response.client.due_date).toLocaleDateString('pt-BR')}`;
      
      if (renewOptions.register_payment && response.transaction) {
        mainMessage += ` | R$ ${response.transaction.amount_received}`;
      }
      
      // Verificar renovação IPTV
      if (renewOptions.renew_in_iptv && response.iptv_renewal) {
        if (response.iptv_renewal.success) {
          if (client.is_unitv_plan) {
            toast.success(`Código UniTV entregue! ${mainMessage}`, { duration: 6000 });
          } else {
            toast.success(`Renovado no ${integrationName}! ${mainMessage}`, { duration: 6000 });
          }
        } else if (response.iptv_renewal.skipped) {
          if (client.is_unitv_plan && response.iptv_renewal.reason === 'no_codes_available') {
            toast.warning(`Renovado, mas sem códigos UniTV disponíveis. ${mainMessage}`, { duration: 6000 });
          } else {
            toast.warning(`Renovado, mas ${integrationName} pulado. ${mainMessage}`, { duration: 6000 });
          }
        } else {
          toast.warning(`Renovado, mas falha no ${integrationName}. ${mainMessage}`, { duration: 6000 });
        }
      } else {
        toast.success(`Cliente renovado! ${mainMessage}`, { duration: 5000 });
      }
    ...
    h('div', { 
      ...
      h('div', { className: 'p-4 space-y-3' },
        ...
        h('div', { className: 'space-y-2' },
          ...
          h('div', { 
            className: 'flex items-start gap-2 p-2.5 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition',
            onClick: () => setRenewOptions({...renewOptions, register_payment: !renewOptions.register_payment})
          },
            h(CustomCheckbox, {
          ...
          hasIntegration ? 
            h('div', { 
              className: 'flex items-start gap-2 p-2.5 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition border-green-300 bg-green-50',
              onClick: () => setRenewOptions({...renewOptions, renew_in_iptv: !renewOptions.renew_in_iptv})
            },
              h(CustomCheckbox, {
