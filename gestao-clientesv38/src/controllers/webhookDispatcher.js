/* ========================================
   ...
   🆕 RETRY SYSTEM: Salva na fila quando falha
   ======================================== */

import axios from 'axios';
import { query } from '../config/database.js';
import { deliverCodeToClient } from '../services/unitvDeliveryService.js';
import { logRenewal } from '../services/activityLogService.js';
...
async function dispatchRenewalWebhook(clientData) {
  try {
    ...
    console.log(`   Is PainelFoda: ${clientData.is_painelfoda_plan || false}`);  // ← ADICIONAR
    console.log(`   Is Rush: ${clientData.is_rush_plan || false}`);
    console.log(`   Rush Type: ${clientData.rush_type || 'N/A'}`);
    console.log(`   Username: ${clientData.username || '(vazio)'}`);
    console.log(`   Suffix: ${clientData.suffix || '(vazio)'}`);
      ...
      try {
        ...
        if (deliveryResult.success) {
          ...
          await logRenewal({
            ...
            provider: 'UniTV',
            success: true,
            months: clientData.duration_months || 1,
            details: { codes: deliveryResult.codes }
          });
          ...
          // Log apenas se não foi por falta de integração
          if (deliveryResult.reason !== 'no_integration') {
            await logRenewal({
              userId: clientData.user_id,
              clientId: clientData.id,
              clientName: clientData.name,
              provider: 'UniTV',
              success: false,
              errorMessage: `Pulado: ${deliveryResult.reason}`
            });
          }
    ...
    if (!clientData.is_sigma_plan && 
        ...
        !clientData.is_club_plan &&
        !clientData.is_painelfoda_plan &&
        !clientData.is_rush_plan) {  // ← ADICIONAR
    ...
    const webhookPayload = {
      ...
      club_client_id: clientData.username,
      
      // 🆕 NOVOS CAMPOS: username e suffix separados (para Uniplay/Rush/PainelFoda)
      ...
      is_painelfoda_plan: clientData.is_painelfoda_plan || false,  // ← ADICIONAR
      ...
      // Rush
      is_rush_plan: clientData.is_rush_plan || false,
      rush_type: clientData.rush_type || null,

      // Dados Koffice
      koffice_domain: clientData.koffice_domain || null,

      // Dados PainelFoda (adicionar DEPOIS dos dados do Club)
      painelfoda_domain: clientData.painelfoda_domain || null,
      painelfoda_username: clientData.painelfoda_username || null,
      painelfoda_password: clientData.painelfoda_password || null,
      painelfoda_package_id: clientData.painelfoda_package_id || null,
    ...
    console.log(`   - PainelFoda: ${webhookPayload.is_painelfoda_plan}`);  // ← ADICIONAR
    console.log(`   - Rush: ${webhookPayload.is_rush_plan} (${webhookPayload.rush_type || 'N/A'})`);
    ...
    // Determinar provider para o log de erro
    const provider = clientData.is_sigma_plan ? 'Sigma' :
                     clientData.is_live21_plan ? 'CloudNation' :
                     clientData.is_koffice_plan ? 'Koffice' :
                     clientData.is_uniplay_plan ? 'Uniplay' :
                     clientData.is_unitv_plan ? 'UniTV' :
                     clientData.is_club_plan ? 'Club' :
                     clientData.is_painelfoda_plan ? 'PainelFoda' :
                     clientData.is_rush_plan ? 'Rush' :
                     'Sistema';

    // ✅ NOVO: Registrar ERRO no activity_logs
    try {
      await logRenewal({
        userId: clientData.user_id,
        clientId: clientData.id,
    ...
    try {
      const webhookPayload = {
        ...
        is_koffice_plan: clientData.is_koffice_plan || false,
        is_uniplay_plan: clientData.is_uniplay_plan || false,
        is_unitv_plan: clientData.is_unitv_plan || false,
        is_club_plan: clientData.is_club_plan || false,
        is_painelfoda_plan: clientData.is_painelfoda_plan || false,
        is_rush_plan: clientData.is_rush_plan || false,
        rush_type: clientData.rush_type || null,
        sigma_domain: clientData.sigma_domain || null,
        sigma_plan_code: clientData.sigma_plan_code || null,
      ...
      // Primeira tentativa: retry em 5 minutos
      const nextRetryAt = new Date(Date.now() + 5 * 60 * 1000);

      await query(`
        INSERT INTO renewal_retry_queue 
        (user_id, client_id, client_name, provider, webhook_payload, next_retry_at, status, last_error, attempts)
      ...
      console.log('🔄 [RETRY] Renovação adicionada à fila de retry');
      console.log(`   Próxima tentativa: ${nextRetryAt.toLocaleString('pt-BR')}`);

    } catch (retryError) {
      console.error('❌ Erro ao salvar na fila de retry:', retryError);
    }

    return { 
      success: false, 
      error: error.message,
      note: 'Pagamento foi processado mas renovação automática falhou. Será tentado novamente automaticamente.',
