...
export async function getPaymentSettings(req, res) {
  try {
    ...
    res.json({
      ...
      pix_key_holder: settings.pix_key_holder || null,
...
// ========== BUSCAR CHAVE PIX DO USUÁRIO (PARA TEMPLATES) ==========
export async function getUserPixKey(userId) {
  try {
    const result = await query(`
      SELECT 
        pix_key,
...
// ========== SALVAR CHAVE PIX ==========
export async function savePixKey(req, res) {
  try {
    const userId = req.user.id;
    const { pix_key, pix_key_holder, pix_key_type } = req.body;
    
    if (!pix_key) {
      return res.status(400).json({ error: 'Chave PIX é obrigatória' });
    }
    
    console.log('💾 Salvando chave PIX para user:', userId);
    ...
    if (existingResult.rows.length > 0) {
      ...
      await query(`
        ...
        SET 
          ...
          updated_at = NOW()
        WHERE user_id = $4
      `, [pix_key, pix_key_holder || null, pix_key_type || null, userId]);
      
      console.log('✅ Chave PIX atualizada!');
    } else {
      // INSERT com apenas PIX (sem MP)
      await query(`
        INSERT INTO payment_settings (
          user_id,
          mercadopago_enabled,
      ...
      `, [userId, false, pix_key, pix_key_holder || null, pix_key_type || null]);
...
export async function getUserCredentials(userId) {
  try {
    ...
    return {
      access_token: settings.mercadopago_access_token ? decrypt(settings.mercadopago_access_token) : null,
      public_key: settings.mercadopago_public_key ? decrypt(settings.mercadopago_public_key) : null,
      payment_domain: settings.payment_domain,
