...
export async function sendNow(req, res) {
  try {
    ...
    // 🆕 Buscar dados do PIX do usuário
    const userPixData = await getUserPixData(req.user.id);
    console.log(`🔑 Dados PIX: ${userPixData ? 'Configurado' : 'Não configurado'}`);
    console.log(`   🔍 DEBUG userPixData:`, JSON.stringify(userPixData));
    console.log(`   🔍 DEBUG pixData:`, JSON.stringify(pixData));
    ...
    for (const reminder of reminders) {
      ...
      // 🆕 Buscar clientes que se enquadram HOJE (COM payment_type)
      let clientQuery = `
        SELECT 
          c.id as client_id,
          c.name,
      ...
      for (const client of clients) {
        ...
        console.log(`   💳 ${client.name}: ${client.payment_type || 'link'} ${pixData ? '(PIX configurado)' : ''}`);

        // 🆕 Processar template com variáveis (passando pixData)
        const message = replaceVariables(reminder.template_message, client, null, pixData);
...
export async function sendReminderToClient(req, res) {
  try {
    ...
    // ========== 1. BUSCAR CLIENTE (🆕 COM payment_type) ==========
    ...
    // 🆕 Buscar dados do PIX do usuário
    const userPixData = await getUserPixData(userId);
    
    // 🆕 Preparar dados do PIX (apenas para clientes tipo 'pix')
    const pixData = (client.payment_type === 'pix') ? userPixData : null;
    
    console.log(`   🔑 Dados PIX: ${pixData ? 'Usando' : 'Não aplicável'}`);
    console.log(`   🔍 DEBUG userPixData:`, JSON.stringify(userPixData));
    console.log(`   🔍 DEBUG pixData:`, JSON.stringify(pixData));
    ...
    for (const reminder of reminders) {
      ...
      // 🆕 Substituir variáveis (passando pixData)
