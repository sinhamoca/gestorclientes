...
// Serve a pasta /user-system/images em /images
// Caminho absoluto para funcionar com Docker
app.use('/images', express.static('/user-system/images'));
console.log('✅ Pasta de imagens configurada: /images');
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      frameSrc: [
        "'self'",
        "https://sdk.mercadopago.com",
        "https://secure-fields.mercadopago.com",  // ← ADICIONAR ESTE
        "https://*.mercadopago.com",               // ← ADICIONAR ESTE (wildcard)
...
app.post('/api/clients/:id/send-invoice', verifyToken, clientsController.sendInvoiceToClient);
...
// ========== ENVIO IMEDIATO (ADICIONAR ESTAS 2 LINHAS) ==========
app.get('/api/reminders/preview-send-now', verifyToken, remindersController.previewSendNow);
app.post('/api/reminders/send-now', verifyToken, remindersController.sendNow);

// ========== 🆕 ENVIAR LEMBRETE PARA CLIENTE ESPECÍFICO ==========
app.post('/api/reminders/send-to-client/:clientId', verifyToken, remindersController.sendReminderToClient);
...
// 🆕 Rotas de notificação de retry
app.get('/api/whatsapp/notification-settings', verifyToken, whatsappController.getNotificationSettings);
app.post('/api/whatsapp/notification-settings', verifyToken, whatsappController.saveNotificationSettings);

// 🆕 Rotas de fila de retry
app.get('/api/whatsapp/retry-queue', verifyToken, whatsappController.getRetryQueue);
app.post('/api/whatsapp/retry-queue/:id/retry', verifyToken, whatsappController.manualRetry);
app.delete('/api/whatsapp/retry-queue/:id', verifyToken, whatsappController.removeFromRetryQueue);

// 🆕 Rotas de configurações de chamadas e presence
app.get('/api/whatsapp/call-settings', verifyToken, whatsappController.getCallSettings);
app.post('/api/whatsapp/call-settings', verifyToken, whatsappController.saveCallSettings);
...
app.post('/api/payment-settings/pix', verifyToken, paymentSettingsController.savePixKey);
...
// ========== ROTAS DE LOGS DE ATIVIDADES ==========
app.get('/api/activity-logs', verifyToken, activityLogController.listActivityLogs);
app.get('/api/activity-logs/stats', verifyToken, activityLogController.getActivityStats);
app.delete('/api/activity-logs/cleanup', verifyToken, activityLogController.cleanupOldLogs);
