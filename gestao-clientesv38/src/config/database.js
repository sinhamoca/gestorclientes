...
export async function initDatabase() {
  try {
    ...
    // ========== ADICIONAR CAMPO skip_number_validation ==========
    await query(`
      DO $$ 
      BEGIN
        -- skip_number_validation (desabilita tratamento de número BR)
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'clients' AND column_name = 'skip_number_validation'
        ) THEN
          ALTER TABLE clients ADD COLUMN skip_number_validation BOOLEAN DEFAULT false;
          RAISE NOTICE '✅ Coluna skip_number_validation adicionada à tabela clients';
        END IF;
      END $$;
    `);
    console.log('🌍 Campo skip_number_validation verificado na tabela clients');
    ...
    // ========== 🔄 TABELA DE RETRY DE RENOVAÇÃO ==========
    await query(`
      CREATE TABLE IF NOT EXISTS renewal_retry_queue (
        id SERIAL PRIMARY KEY,
        user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
        client_id INTEGER REFERENCES clients(id) ON DELETE CASCADE,
    ...
    // Índices para renewal_retry_queue
    await query(`CREATE INDEX IF NOT EXISTS idx_retry_queue_status ON renewal_retry_queue(status);`);
    await query(`CREATE INDEX IF NOT EXISTS idx_retry_queue_next_retry ON renewal_retry_queue(next_retry_at);`);
    await query(`CREATE INDEX IF NOT EXISTS idx_retry_queue_user_id ON renewal_retry_queue(user_id);`);
    await query(`CREATE INDEX IF NOT EXISTS idx_retry_queue_status_next ON renewal_retry_queue(status, next_retry_at);`);

    console.log('🔄 Tabela renewal_retry_queue criada');

    // ========== 🔔 COLUNAS DE NOTIFICAÇÃO NO WHATSAPP_INSTANCES ==========
    await query(`
      DO $$ 
      BEGIN
        -- notify_renewal_failures: Habilitar notificações de falha
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'notify_renewal_failures'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN notify_renewal_failures BOOLEAN DEFAULT false;
          RAISE NOTICE '✅ Coluna notify_renewal_failures adicionada';
        END IF;
        
        -- notify_phone_number: Número para receber notificações
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'notify_phone_number'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN notify_phone_number VARCHAR(20);
          RAISE NOTICE '✅ Coluna notify_phone_number adicionada';
        END IF;
        
        -- notify_on_retry_success: Notificar quando retry funcionar
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'notify_on_retry_success'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN notify_on_retry_success BOOLEAN DEFAULT true;
          RAISE NOTICE '✅ Coluna notify_on_retry_success adicionada';
        END IF;
        
        -- notify_on_each_retry: Notificar em cada tentativa (opcional)
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'notify_on_each_retry'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN notify_on_each_retry BOOLEAN DEFAULT false;
          RAISE NOTICE '✅ Coluna notify_on_each_retry adicionada';
        END IF;
      END $$;
    `);

    console.log('🔔 Colunas de notificação adicionadas em whatsapp_instances');

    // ========== 📞 COLUNAS DE REJECT CALLS E ALWAYS ONLINE ==========
    await query(`
      DO $$ 
      BEGIN
        -- auto_reject_calls: Recusar chamadas automaticamente
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'auto_reject_calls'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN auto_reject_calls BOOLEAN DEFAULT false;
          RAISE NOTICE '✅ Coluna auto_reject_calls adicionada';
        END IF;
        
        -- reject_call_message: Mensagem ao recusar chamada
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'reject_call_message'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN reject_call_message TEXT DEFAULT 'Desculpe, não recebo chamadas por aqui. Me envie uma mensagem! 📱';
          RAISE NOTICE '✅ Coluna reject_call_message adicionada';
        END IF;
        
        -- always_online: Ficar sempre online
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'whatsapp_instances' AND column_name = 'always_online'
        ) THEN
          ALTER TABLE whatsapp_instances ADD COLUMN always_online BOOLEAN DEFAULT false;
          RAISE NOTICE '✅ Coluna always_online adicionada';
        END IF;
      END $$;
    `);

    console.log('📞 Colunas de reject calls e always online adicionadas');
  ...
  // ========================================
    ...
    console.log('   🔄 Tabelas criadas: renewal_retry_queue');
