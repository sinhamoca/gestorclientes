...
export async function createPlan(req, res) {
  try {
    const { name, duration_months, num_screens } = req.body;
    ...
    const result = await query(
      `INSERT INTO plans (user_id, name, duration_months, num_screens)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [req.user.id, name, duration_months, num_screens]
    );
...
export async function updatePlan(req, res) {
  try {
    ...
    const result = await query(
      `UPDATE plans 
       SET name = $1, duration_months = $2, num_screens = $3
       WHERE id = $4 AND user_id = $5
       RETURNING *`,
      [name, duration_months, num_screens, id, req.user.id]
    );
