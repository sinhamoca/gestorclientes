...
export async function createPlan(req, res) {
  try {
    const { name, duration_months, num_screens, is_sigma_plan, sigma_plan_code } = req.body;
    ...
    const result = await query(
      `INSERT INTO plans (user_id, name, duration_months, num_screens, is_sigma_plan, sigma_plan_code)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [req.user.id, name, duration_months, num_screens, is_sigma_plan || false, sigma_plan_code || null]
    );
...
export async function updatePlan(req, res) {
  try {
    ...
    const result = await query(
      `UPDATE plans 
       SET name = $1, duration_months = $2, num_screens = $3, is_sigma_plan = $4, sigma_plan_code = $5
       WHERE id = $6 AND user_id = $7
       RETURNING *`,
      [name, duration_months, num_screens, is_sigma_plan || false, sigma_plan_code || null, id, req.user.id]
    );
