...
const api = {
  ...
  /**
   * Sincronizar cliente com Club (NOVO!)
   */
  async syncClientWithClub(clientId, clubClientId) {
    const token = auth.getToken();
    
    try {
      const response = await fetch(`${IPTV_API_URL}/clients/${clientId}/sync-club`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao sincronizar cliente com Club');
      }

      return data;
    } catch (error) {
      console.error('❌ [API] Erro ao sincronizar cliente com Club:', error);
      throw error;
    }
  },
