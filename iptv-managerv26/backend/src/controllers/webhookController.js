 ...
 * VERSÃO ATUALIZADA COM SESSION KEEPER
 * 
 * @param {Request} req 
 * @param {Response} res 
 * @param {Object} webhookData - Dados do webhook
 * @param {Object} plan - Plano encontrado
 */
async function handleCloudNationRenewal(req, res, webhookData, plan) {
  ...
  console.log('\n🔵 ========================================');
  console.log('   RENOVAÇÃO CLOUDNATION');
  console.log('========================================');
  console.log(`   👤 Cliente: ${client_name}`);
  console.log(`   🆔 CloudNation ID(s): ${cloudnation_id}`);
  console.log(`   📅 Meses: ${plan_duration_months}`);
  console.log(`   🔧 Modo: ${process.env.CLOUDNATION_RENEWAL_MODE || 'legacy'}`);

  // Validar se tem CloudNation ID
  if (!cloudnation_id) {
    console.log('❌ [WEBHOOK-CN] Cliente sem CloudNation ID configurado');
    return res.status(400).json({
      success: false,
      error: 'Cliente sem CloudNation ID configurado'
    });
  }

  // Suporte a múltiplos IDs (separados por vírgula)
  const cloudnationClientIds = cloudnation_id
    .split(',')
    .map(id => id.trim())
    .filter(id => id.length > 0);
  
  const totalClientes = cloudnationClientIds.length;
  
  console.log(`   👥 Total de clientes para renovar: ${totalClientes}`);

  // Buscar credenciais (mesma função existente no sistema)
  const credentials = db.getCredentials(user_id);
  
  if (!credentials) {
    console.log('❌ [WEBHOOK-CN] Credenciais não encontradas');
    return res.status(400).json({
      success: false,
  ...
  // Decodificar senha (está em base64)
  ...
  try {
    // ========== RENOVAÇÃO USANDO FACTORY ==========
    // A factory decide se usa KEEPER ou LEGACY baseado no ENV
    
    if (totalClientes === 1) {
      // Cliente único
      const result = await cloudnationFactory.renewClient({
        userId: user_id,
        credentials: decodedCreds,
        clientId: cloudnationClientIds[0],
        months: plan_duration_months
      });

      if (result.success) {
        console.log(`✅ [WEBHOOK-CN] Cliente ${cloudnationClientIds[0]} renovado com sucesso!`);
        console.log(`   📊 Modo utilizado: ${result.mode?.toUpperCase() || 'N/A'}`);
        
        return res.json({
          success: true,
          provider: 'cloudnation',
          mode: result.mode,
          client_id: cloudnationClientIds[0],
          months: plan_duration_months,
          message: `Cliente renovado por ${plan_duration_months} mês(es)`
        });
      } else {
        throw new Error(result.error || 'Falha na renovação');
      }
      
    } else {
      // Múltiplos clientes
      const result = await cloudnationFactory.renewMultipleClients({
        userId: user_id,
        credentials: decodedCreds,
        clientIds: cloudnationClientIds,
        months: plan_duration_months
      });

      console.log(`✅ [WEBHOOK-CN] Lote concluído: ${result.success}/${result.total} renovados`);
      console.log(`   📊 Modo utilizado: ${result.mode?.toUpperCase() || 'N/A'}`);

      return res.json({
        success: result.success === result.total,
        partial: result.success > 0 && result.success < result.total,
        provider: 'cloudnation',
        mode: result.mode,
        total: result.total,
        ...
        message: `${result.success}/${result.total} clientes renovados`
      });
    }

  } catch (error) {
    console.error('❌ [WEBHOOK-CN] Erro na renovação:', error.message);
    
    return res.status(500).json({
      success: false,
      provider: 'cloudnation',
      error: error.message,
      message: 'Falha ao renovar cliente no painel CloudNation'
    });
  }
}
...
async function handleKofficeRenewal(req, res, webhookData, plan) {
  const { 
    ...
    user_id, 
    client_name, 
    cloudnation_id,  // Usado como koffice_client_id
    plan_duration_months
  } = webhookData;

  console.log('\n🟠 ========================================');
  console.log('   RENOVAÇÃO KOFFICE');
  console.log('========================================');
  console.log(`   👤 Cliente: ${client_name}`);
  console.log(`   🆔 Koffice Client ID(s): ${cloudnation_id}`);
  console.log(`   🌐 Domínio: ${plan.koffice_domain}`);
  console.log(`   📅 Meses: ${plan_duration_months}`);
  console.log(`   🔧 Modo: ${process.env.KOFFICE_RENEWAL_MODE || 'legacy'}`);

  // Validar se tem Client ID
  if (!cloudnation_id) {
    console.log('❌ [WEBHOOK-KOFFICE] Cliente sem Koffice ID configurado');
    return res.status(400).json({
      success: false,
      error: 'Cliente sem Koffice ID configurado'
    });
  }
  ...
  // Buscar credenciais (necessário para modo legacy e para validação)
  const credentials = db.getKofficeCredentialByDomain(user_id, plan.koffice_domain);
  
  if (!credentials) {
    console.log('❌ [WEBHOOK-KOFFICE] Credenciais não encontradas para este domínio');
    return res.status(400).json({
      success: false,
      error: 'Credenciais Koffice não encontradas para este domínio'
    });
  }
  ...
  try {
    ...
    if (totalClientes === 1) {
      // Cliente único
      const result = await kofficeFactory.renewClient({
        userId: user_id,
        domain: plan.koffice_domain,
        credentials,
        clientId: kofficeClientIds[0],
        months: plan_duration_months
      });

      if (result.success) {
        console.log(`✅ [WEBHOOK-KOFFICE] Cliente ${kofficeClientIds[0]} renovado com sucesso!`);
        console.log(`   📊 Modo utilizado: ${result.mode?.toUpperCase() || 'N/A'}`);
      ...
      // Múltiplos clientes
      const result = await kofficeFactory.renewMultipleClients({
        userId: user_id,
        domain: plan.koffice_domain,
        credentials,
        clientIds: kofficeClientIds,
        months: plan_duration_months
      });

      console.log(`✅ [WEBHOOK-KOFFICE] Lote concluído: ${result.success}/${result.total} renovados`);
      console.log(`   📊 Modo utilizado: ${result.mode?.toUpperCase() || 'N/A'}`);
    ...
    console.error('❌ [WEBHOOK-KOFFICE] Erro na renovação:', error.message);
    
    return res.status(500).json({
      success: false,
      provider: 'koffice',
      error: error.message,
      message: 'Falha ao renovar cliente no painel Koffice'
    });
  }
}
