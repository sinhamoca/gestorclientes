// gestao-clientesv2/src/controllers/usersController.js
  ...
  try {
    ...
    let sql = `
      SELECT id, name, email, phone, role, subscription_start, subscription_end, 
             is_active, max_clients, max_instances, messages_per_minute, created_at
      FROM users 
      WHERE 1=1
    `;
    const params = [];
...
export async function getUser(req, res) {
  try {
    ...
    const result = await query(
      `SELECT id, name, email, phone, role, subscription_start, subscription_end, 
              is_active, max_clients, max_instances, messages_per_minute, created_at, updated_at
    ...
    const result = await query(
      `INSERT INTO users 
       ...
       RETURNING id, name, email, phone, subscription_start, subscription_end, is_active, 
                 max_clients, max_instances, messages_per_minute`,
      [
        name, 
        email, 
        passwordHash, 
...
export async function updateUser(req, res) {
  try {
    ...
    if (messages_per_minute) {
      updates.push(`messages_per_minute = $${paramCount++}`);
      values.push(messages_per_minute);
    }
    ...
    const sql = `
      ...
      RETURNING id, name, email, phone, subscription_start, subscription_end, 
                is_active, max_clients, max_instances, messages_per_minute
    `;
...
export async function getUsersWithClientCount(req, res) {
  try {
    const result = await query(`
      SELECT u.id, u.name, u.email, u.subscription_end, u.is_active, u.messages_per_minute,
