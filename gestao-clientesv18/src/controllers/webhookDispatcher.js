/* ========================================
   WEBHOOK DISPATCHER - COM VALIDAÇÃO UNIPLAY
   Dispara webhook para IPTV Manager APENAS se o plano tiver renovação automatizada
   ATUALIZADO: Suporte Sigma, Live21, Koffice e Uniplay
   ======================================== */
 ...
 * 
 * SUPORTA: Sigma, Live21/CloudNation, Koffice e Uniplay
 */
async function dispatchRenewalWebhook(clientData) {
  try {
    ...
    // ========== VALIDAÇÃO 5: Plano Uniplay (renovação via API Uniplay) ← NOVO ==========
    if (clientData.is_uniplay_plan) {
      ...
      // Uniplay não precisa validações extras
      // Usa apenas o NOME do cliente para buscar e renovar
    }
    ...
    const webhookPayload = {
      ...
      client_name: clientData.name,  // ← Usado pelo Uniplay para buscar
      whatsapp_number: clientData.whatsapp_number,
      
      // IDs de renovação (usado pelos sistemas)
      cloudnation_id: clientData.username,        // CloudNation ID
      sigma_customer_id: clientData.username,     // Sigma ID
      koffice_client_id: clientData.username,     // Koffice Client ID
      // Uniplay usa client_name diretamente (não precisa de ID)
      
      // Informações do plano
      plan_duration_months: clientData.duration_months || 1,  // ← Uniplay: créditos
      ...
      is_uniplay_plan: clientData.is_uniplay_plan || false,  // ← NOVO
      
      // Dados Sigma (se aplicável)
      sigma_domain: clientData.sigma_domain || null,
      sigma_plan_code: clientData.sigma_plan_code || null,
      
      // Dados Koffice (se aplicável)
      koffice_domain: clientData.koffice_domain || null,
      
      // Dados Uniplay: usa apenas o nome (já incluído em client_name)
      // Domínio fixo: gesapioffice.com (hardcoded no service)
    ...
    console.log(`   - Uniplay: ${webhookPayload.is_uniplay_plan}`);  // ← NOVO
