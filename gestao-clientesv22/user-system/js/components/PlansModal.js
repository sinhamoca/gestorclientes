/* ========================================
   PLANS MODAL COMPONENT - COM EDIÇÃO, PLANO SIGMA E PLANO LIVE21
   ======================================== */
...
function PlansModal({ plans, onClose, onRefresh }) {
  ...
  const handleEdit = (plan) => {
    setFormData({
      ...
      is_koffice_plan: plan.is_koffice_plan || false,  // ← NOVO
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto">
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            {/* Checkbox Plano Live21 - NOVO! */}
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formData.is_live21_plan}
                  onChange={(e) => setFormData({...formData, is_live21_plan: e.target.checked})}
                  className="mt-1 w-4 h-4"
                />
                <div className="flex-1">
            ...
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setFormData({...formData, is_koffice_plan: e.target.checked})}
                  className="mr-3 w-5 h-5"
                />
                <div className="flex-1">
                  <span className="font-medium text-lg">Plano Koffice?</span>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...
                      // Desmarcar outros tipos quando Uniplay for marcado
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    {formData.is_uniplay_plan ? (
                      <span className="text-blue-700">
                        ...
                        <br/>
                        • Domínio fixo: <code className="bg-blue-100 px-1 rounded">gesapioffice.com</code>
                        <br/>
                        • Identificação: Nome do cliente
                        <br/>
                        • Créditos: Campo "Duração (meses)" = créditos (1 mês = 1 crédito)
                        <br/>
            ...
            <div className="col-span-full">
              <label className="flex items-start gap-3 p-4 border rounded-lg cursor-pointer hover:bg-purple-50 transition">
                <input
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...formData,
                      is_unitv_plan: newValue,
                      // Desmarcar outros tipos se UniTV for marcado
                      is_sigma_plan: newValue ? false : formData.is_sigma_plan,
                      is_live21_plan: newValue ? false : formData.is_live21_plan,
                      is_koffice_plan: newValue ? false : formData.is_koffice_plan,
                      is_uniplay_plan: newValue ? false : formData.is_uniplay_plan
                    });
                  }}
                  className="mt-1"
                />
                <div className="flex-1">
                  <div className="font-medium text-purple-700">🎫 É plano UniTV?</div>
                  <p className="text-sm text-gray-600 mt-1">
                    Plano que entrega <strong>códigos de ativação via WhatsApp</strong> ao invés de renovar em painel IPTV.
                    Cliente recebe código automaticamente após pagamento.
                  </p>
                  {formData.is_unitv_plan && (
                    <div className="mt-2 p-3 bg-purple-50 border border-purple-200 rounded text-sm">
                      <p className="text-purple-800 font-medium">ℹ️ Como funciona:</p>
                      <ul className="list-disc list-inside text-purple-700 mt-1 space-y-1">
                        <li>Cliente paga e sistema busca código disponível</li>
                        <li>Código é enviado automaticamente via WhatsApp</li>
                        <li>Se não houver códigos, cliente é renovado apenas no sistema</li>
                        <li>Gerencie códigos em: Menu → Códigos UniTV</li>
                      </ul>
                    </div>
                  )}
                </div>
            ...
            {formData.is_koffice_plan && (
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  💡 Este domínio será usado para renovação automática via webhook
                </p>
              </div>
            )}
                    ...
                    <div className="text-xs text-purple-600 mt-1 space-y-0.5">
                      ...
                      {plan.koffice_domain && (
                        <div className="text-xs text-gray-500 mt-1">
                          🌐 Domínio Koffice: {plan.koffice_domain}
                        </div>
                      )}
                    </div>
                  )}
