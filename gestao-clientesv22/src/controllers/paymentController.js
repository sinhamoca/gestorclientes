...
export async function getClientPaymentHistory(req, res) {
  try {
    ...
    // Formatar códigos UniTV
    const transactions = historyResult.rows.map(tx => {
      if (tx.unitv_code) {
        // Formatar: 1111111111111111 → 1111-1111-1111-1111
        tx.unitv_code_formatted = tx.unitv_code.match(/.{1,4}/g).join('-');
      }
      return tx;
    });
...
async function processApprovedPayment(payment, session) {
  try {
    ...
    // Debug: ver o que o webhook retornou
    console.log('🔍 [DEBUG] webhookResult completo:', JSON.stringify(webhookResult, null, 2));
    
    // Tentar múltiplas formas de extrair o code_id
    const unitvCodeId = 
      webhookResult.unitv_code_id ||      // Primeiro: direto
      webhookResult.codeId ||              // Segundo: codeId
      webhookResult.data?.unitv_code_id || // Terceiro: dentro de data
      webhookResult.data?.codeId ||        // Quarto: data.codeId
      null;
    
    console.log(`💾 Salvando transação financeira...`);
    console.log(`   🔍 [DEBUG] unitvCodeId extraído: ${unitvCodeId}`);
    
    if (unitvCodeId) {
      console.log(`   🎫 Com código UniTV ID: ${unitvCodeId}`);
    } else {
      console.log(`   ⚠️  Nenhum código UniTV vinculado a esta transação`);
    }
    
    // 5. Registrar transação financeira (COM unitv_code_id)
    await query(`
      INSERT INTO financial_transactions (
        ...
        gateway_payment_id, payment_session_id, unitv_code_id
    ...
    `, [
      ...
      netProfit,
      clientData.due_date,
      // ← CURRENT_TIMESTAMP já está no SQL acima!
      'paid',
  ...
  <script>
    ...
    async function generatePix() {
      try {
        ...
        container.innerHTML = \`
          <h3 style="color: #333; margin-bottom: 15px;">📱 Escaneie o QR Code</h3>
          <img src="data:image/png;base64,\${data.qr_code_base64}" alt="QR Code PIX" />
          <p style="color: #666; font-size: 14px; margin-bottom: 10px;">
            Ou copie o código PIX:
          </p>
          <div style="background: #f5f5f5; padding: 15px; border-radius: 8px; word-break: break-all; font-family: monospace; font-size: 12px;">
            \${data.qr_code}
          </div>
      ...
      try {
        ...
        if (data.transactions && data.transactions.length > 0) {
          ...
          data.transactions.forEach(tx => {
            const date = new Date(tx.paid_date + 'T00:00:00-03:00');
            const dateStr = date.toLocaleDateString('pt-BR', {
              day: '2-digit',
              month: 'long',
              year: 'numeric'
            });
            const timeStr = new Date(tx.paid_date).toLocaleTimeString('pt-BR', {
              hour: '2-digit',
              minute: '2-digit'
            });
            ...
            if (tx.unitv_code_formatted) {
              html += \`
                <div class="unitv-code-box">
                  <div class="unitv-code-title">🎫 Código de Ativação UniTV</div>
                  <div class="unitv-code-display">
                    <div class="unitv-code-value">\${tx.unitv_code_formatted}</div>
                    <button class="unitv-copy-button" onclick="copyUnitvCode('\${tx.unitv_code}')">
                      📋 Copiar Código
                    </button>
                  </div>
                  <div class="unitv-instructions">
                    <strong>📱 Como usar:</strong>
                    <ol>
                      <li>Abra o aplicativo UniTV</li>
                      <li>Entre em "Ativar Código"</li>
