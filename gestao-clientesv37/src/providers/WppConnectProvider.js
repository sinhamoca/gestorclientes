...
class WppConnectProvider extends WhatsAppProvider {
  ...
  async checkNumberExists(sessionId, phoneNumber) {
    try {
      console.log(`🔍 [WPP Connect] Verificando número: ${phoneNumber}`);
      
      const response = await fetch(`${this.apiUrl}/api/number/check`, {
        method: 'POST',
        headers: this.getHeaders(),
        body: JSON.stringify({
          sessionId,
      ...
      if (!response.ok) {
        const error = await response.json();
        console.error(`❌ [WPP Connect] Erro ao verificar:`, error);
        return { exists: false, error: error.error || 'Erro ao verificar' };
      }

      const result = await response.json();
      console.log(`   📋 Resultado: existe=${result.exists}`);
      
      return result;

    } catch (error) {
      console.error(`❌ [WPP Connect] Erro ao verificar número:`, error);
