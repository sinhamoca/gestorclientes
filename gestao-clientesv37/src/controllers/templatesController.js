...
// USA CHAVES SIMPLES: {nome}, {dias}, {fatura}, etc.
export function replaceVariables(message, client, paymentUrl = null) {
  ...
  return message
    ...
    .replace(/\{fatura\}/gi, paymentUrl || 'Link indisponível')
    .replace(/\{link_pagamento\}/gi, paymentUrl || 'Link indisponível'); // alias de {fatura}
}