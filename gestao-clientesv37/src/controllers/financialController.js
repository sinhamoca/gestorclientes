...
export async function getFinancialDashboard(req, res) {
  try {
    ...
    // 1. RECEITA BRUTA - Soma do price_value de todos os clientes que vencem no mês
...
export async function registerPayment(req, res) {
  try {
    ...
    const result = await query(`
      ...
      RETURNING *
    `, [
      userId, client_id, client.name, 'renewal', amountReceived, serverCost, netProfit,
...
// ========== LISTAR TRANSAÇÕES (CORRIGIDO) ==========
// Agora usa LEFT JOIN para mostrar transações mesmo de clientes deletados
export async function listTransactions(req, res) {
  try {
    ...
    let sql = `
      SELECT 
        ft.*,
        COALESCE(c.name, ft.client_name, 'Cliente removido') as client_name,
        c.whatsapp_number
      FROM financial_transactions ft
