...
export async function handleClientRenewalWebhook(req, res) {
  try {
    ...
    if (!plan.is_sigma_plan && !plan.is_live21_plan && !plan.is_koffice_plan && !plan.is_uniplay_plan) {
      ...
      console.log('   is_club_plan: false');  // ← ADICIONAR
...
async function handleClubRenewal(req, res, webhookData, plan) {
  const { 
    ...
    user_id, 
    client_name,
    cloudnation_id,  // Na verdade é o client_id do Club
    plan_duration_months 
  } = webhookData;
  ...
  console.log(`   👤 Cliente: ${client_name}`);
  console.log(`   🆔 Club Client ID: ${cloudnation_id}`);
  ...
  try {
    ...
    // ========== RENOVAR CLIENTE ==========
    const resultado = await service.renovarCliente(cloudnation_id, plan_duration_months);
    
    if (resultado.sucesso) {
      console.log('\n✅ [WEBHOOK-CLUB] RENOVAÇÃO CONCLUÍDA COM SUCESSO!');
      console.log('='.repeat(60) + '\n');
      ...
      return res.json({
        ...
        data: {
          client_id: client_id,
          client_name: client_name,
          club_client_id: cloudnation_id,
          plan_duration_months: plan_duration_months,
          novo_vencimento: resultado.novoVencimento,
