...
export async function createClient(req, res) {
  try {
    ...
    // 🔐 CRIPTOGRAFIA DUPLA
    let whatsappToSave = whatsapp_number;
    let whatsappEncrypted = null; // Camada 1: Chave do usuário (E2E)
    let whatsappInternal = null;  // Camada 2: Chave do sistema (lembretes)

    if (whatsapp_number) {
      try {
        // CAMADA 1: Criptografia E2E (chave do usuário)
        if (req.encryptionKey) {
          whatsappEncrypted = encrypt(whatsapp_number, req.encryptionKey);
        ...
        if (process.env.SYSTEM_ENCRYPTION_KEY) {
          whatsappInternal = encrypt(whatsapp_number, process.env.SYSTEM_ENCRYPTION_KEY);
  ...
  try {
    ...
    const result = await query(
      ...
      SET name = $1, whatsapp_number = $2, whatsapp_number_encrypted = $3, whatsapp_number_internal = $4,
          ...
          notes = $13, is_active = $14, player_type = $15, player_domain = $16, updated_at = NOW()
  ...
  try {
    ...
    if (renew_in_iptv) {
      ...
      if (!client.is_sigma_plan && 
          ...
          !client.is_club_plan &&
          !client.is_painelfoda_plan) {  // ← ADICIONAR
      ...
      } else if (!client.username && !client.is_unitv_plan && !client.is_painelfoda_plan) {
